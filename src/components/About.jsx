export default function About() {
  return (
    <section id="about" className="py-24 px-6 bg-cream dark:bg-ink transition-colors duration-500">
      <div className="max-w-6xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          {/* Text side */}
          <div>
            <p className="text-xs tracking-widest2 uppercase text-muted dark:text-stone font-body mb-6">— Behind the lens</p>
            <h2 className="font-display text-5xl md:text-6xl font-light text-ink dark:text-cream mb-8 leading-tight">
              Hi, I'm<br />
              <em className="italic">Kishan.</em>
            </h2>

            <div className="space-y-4 font-body text-sm text-muted dark:text-stone leading-relaxed">
              <p>
            ...
            <div className="mt-10 grid grid-cols-3 gap-6 border-t border-stone/30 pt-10">
              {[['50+', 'Weddings'], ['200+', 'Photo shoots'], ['100+', 'Video projects']].map(([n, l]) => (
                <div key={l}>
                  <div className="font-display text-4xl font-light text-ink dark:text-cream">{n}</div>
                  <div className="text-xs tracking-widest uppercase text-muted dark:text-stone font-body mt-1">{l}</div>
                </div>
              ))}
            </div>
            ...
            {/* Gear */}
            <div className="mt-10">
              <p className="text-xs tracking-widest uppercase text-muted dark:text-stone font-body mb-3">Gear</p>
              <div className="flex flex-wrap gap-2">
                {['Sony A7 III', '24–70mm f/2.8', '50mm f/1.8', 'DJI Drone', 'Godox Flash', 'Adobe Premiere'].map(g => (
                  <span key={g} className="text-xs border border-stone/50 dark:border-stone/20 px-3 py-1.5 text-muted dark:text-stone font-body">{g}</span>
                ))}
              </div>
            </div>
          </div>
